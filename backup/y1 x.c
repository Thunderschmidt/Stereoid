...
void init_sound()
{
    ...
    sound[1] = load_wav ("sounds/laserwc.wav");
...
{
    player.vorhalt.t[0]=unit[u]->m.t[0]-unit[u]->flightdir.x*t;
    player.vorhalt.t[1]=unit[u]->m.t[1]-unit[u]->flightdir.y*t;
    player.vorhalt.t[2]=unit[u]->m.t[2]-unit[u]->flightdir.z*t;
}
...
void missile_hit_fighter(UNIT *u1, UNIT *u2)
{
    ...
    pushdir.x=col_point.x-u2->m.t[0];
    pushdir.y=col_point.y-u2->m.t[1];
    pushdir.z=col_point.z-u2->m.t[2];
    normalize_vector_f(&pushdir.x, &pushdir.y, &pushdir.z);
    u2->flightdir.x+=pushdir.x*290/u2->udata->mass;;
    u2->flightdir.y+=pushdir.y*290/u2->udata->mass;;
    u2->flightdir.z+=pushdir.z*290/u2->udata->mass;;
    MATRIX_f m=u2->m;
...
{
    VECTOR col_point;
    u->model->colmodel->getCollisionPoint ( &col_point.x, 0); //ermittle Kollisionspunkt
    MATRIX_f m=u->m;
    translate_matrix_v(&m, &col_point);           //erstelle Matrix für Explosion
    start_explosion(&m, &u->speed, sht->property);
    m.v[2][0]=col_point.x+sht->m.v[2][0];         //spanne Ebene auf (Kollisionspunkt, Mittelpunkt, Schuß-Vektor)
    ...
    matrix_mul_f(&m, &u->m, &u->m); //falsch!!! Hier wird gleich der ist-Wert verändert!
...
{
    int i;
    int o=get_object_by_name(unit[u]->model, "engine");
    for (i=0; i<MAXENGINE; i++)
    if (unit[u]->engine[i].active)
    {
 //  	   glColor3ubv(col[c][15-((chrono-unit[u]->engine[i].t)/25)]);  // 	 glColor3ubv(col[c][15]);
       glPushMatrix();
       glMultMatrix_allegro(&unit[u]->engine[i].m);
       draw_object(&unit[u]->model->o3d[o], c);
       glPopMatrix();
    }
}
...
void render_pipeline(char c)
{
	...
	for (i=0;i<RL.u.anz;i++)
	{
       ...
       if(unit[RL.u.i[i]]->type==UNIT_FIGHTER && (unit[RL.u.i[i]]->status != AI_PLAYER))
    ...
    {   
       if ( chrono-unit[u]->shotdelay > unit[u]->udata->shootrate)
       {
          if(unit[u]->gun)
          {
             ...
             raptor_fire_left_gun(unit[u]);
          }
          else
          {
             start_shot(u, "remit", SHOT_LASER);
             raptor_fire_right_gun(unit[u]);
          }
