...
{
    int x,y,z;
    float l=vector_length_f(CROSSBOX/2*CROSSGRID, CROSSBOX/2*CROSSGRID, CROSSBOX/2*CROSSGRID);
    float temp;
    int c=CROSSBOX;
    for(x=0;x<c;x++)
    for(y=0;y<c;y++)
    for(z=0;z<c;z++)
    {
       crosses[x*c*c+y*c+z].x=(x-c/2)*CROSSGRID;
       crosses[x*c*c+y*c+z].y=(y-c/2)*CROSSGRID;
       crosses[x*c*c+y*c+z].z=(z-c/2)*CROSSGRID;
       temp=12-(vector_length_f(crosses[x*c*c+y*c+z].x,crosses[x*c*c+y*c+z].y,crosses[x*c*c+y*c+z].z)/l)*10;
       crosscolor[x*c*c+y*c+z]=temp;
    }
}


void draw_crosses(bool c)
{
    int i, c3=CROSSBOX*CROSSBOX*CROSSBOX;
     for(i=0;i<c3;i++)
    {
      glPushMatrix();
      glTranslatef(crosspos.x ,crosspos.y, crosspos.z);
      glTranslatef(crosses[i].x ,crosses[i].y, crosses[i].z);
      glScalef(CROSSSIZE, CROSSSIZE, CROSSSIZE);
      glColor3ubv(col[c][crosscolor[i]]);
      glCallList(draw_cross_list);
      glPopMatrix();
    }
}
...
bool ray_collision_check2(short u, short i)
{
    VECTOR vo;
    vo.x=missile[i].m.v[2][0];
    vo.y=missile[i].m.v[2][1];
    vo.z=missile[i].m.v[2][2];
    get_translation_matrix_f(&tm0, unit[u].pos.x, unit[u].pos.y, unit[u].pos.z);
    matrix_mul_f(&unit[u].m, &tm0, &tm0);
    allegro_gl_MATRIX_f_to_GLfloat(&tm0, glm0);
    colmodel[unit[u].model]->setTransform (glm0);
    return(colmodel[unit[u].model]->rayCollision (&missile[i].pos.x, &vo.x, 0, 0, missile[i].speed*fmult));
 //   return(colmodel[unit[u].model]->sphereCollision(&missile[i].pos.x, 0.2));
}      
...
void the_timer(void)
{
	...
	if(Recorder)
	{  // Recorder on 
        if(Count >= REC_SCAN_RATE)
        {  // Time to save/get UserData
	        Count = 0;
            switch(Recorder)
	        {
	            case STAT_PLAY:
	                // Get data from file
                    fread(&dat, sizeof(dat), 1, record_file);
	                break;
	           
	            case STAT_REC:
                    dat.Num = chrono;
                    // Record data to file
                    dat.shippos=unit[0].pos;
                    dat.camm=camera.m;
                    fwrite(&dat, sizeof(dat), 1, record_file);
	                break;
	        }
        }
	    else Count++;
...
void init_new_unit()
{
   ...
   for (u=0;u<MAXUNITS;u++)
   {
	...
	unit[u].pos.y = 300;
	unit[u].pos.z = -1680;
    //     unit[u].pos.x = rand()%1000-500;
     //    unit[u].pos.y = rand()%1000-500;
      //   unit[u].pos.z = rand()%1000-500;
...
void make_RL()
{
    ...
    RL.shot.anz=i2; i2=0;
    for(i=0; i<AL.missile.anz; i++)
    {
       if (missile[AL.missile.i[i]].heading.z > 0)
       {
          if (missile[AL.missile.i[i]].heading.x < XHEADING)
          if (missile[AL.missile.i[i]].heading.x >-XHEADING)
          if (missile[AL.missile.i[i]].heading.y < YHEADING)
          if (missile[AL.missile.i[i]].heading.y >-YHEADING)
          {    
             RL.missile.i[i2]=AL.missile.i[i];
             i2++;
          }
       }
    }
...
void get_headings()
{
    ...
    for (i=0; i<AL.missile.anz; i++)
    {
       get_heading(&unit[0].m, &camera.pos, &missile[AL.missile.i[i]].pos, &missile[i].heading);
    }
}
...
//    glScalef(explosion[j].mult/2, explosion[j].mult/2, explosion[j].mult/2);
//    glDrawElements(GL_LINE_LOOP, 36, GL_UNSIGNED_BYTE, circleindex);
...
void collision_check()
{
    ...
    for(i=0; i<AL.missile.anz; i++)
    {
       for(u=0;u<AL.u.anz;u++)
       {
          if (missile[AL.missile.i[i]].property!=AL.u.i[u])
          if (check_col(&missile[AL.missile.i[i]].pos, &unit[AL.u.i[u]].pos, model[unit[AL.u.i[u]].model].size/2))
          {
             if(ray_collision_check2(AL.u.i[u], AL.missile.i[i]))
  		     {
             start_explode(&missile[AL.missile.i[i]].pos, &unit[AL.u.i[u]].m, &unit[AL.u.i[u]].speed_ist.z, missile[AL.missile.i[i]].property);
             unit[AL.u.i[u]].justhit=chrono;
             missile[AL.missile.i[i]].active=0;
             play_3D_sound(&missile[AL.missile.i[i]].pos, sound[5]);
  		     if(unit[AL.u.i[u]].vulnerable)
             {
                unit[AL.u.i[u]].shield-=MISSILEDAMAGE;
                check_unit_death(AL.u.i[u]);
             }
             break;
             }
...
void start_missile(short target, bool seeker)
{
    MATRIX_f m=identity_matrix_f;
    invert_matrix(&camera.m, &m);
 
    int i;
    play_sample (sound[4], 32, 128, 1000, 0);
	for (i=0; i<MAXMISSILE; i++)
	{
		if (!missile[i].active)
		{
  			missile[i].active=1;
  			missile[i].pos.x=unit[0].lemit.x;
  			missile[i].pos.y=unit[0].lemit.y;
  			missile[i].pos.z=unit[0].lemit.z;
  			missile[i].speed=unit[0].speed_ist.z+MISSILESTARTSPEED;
  			missile[i].m=m;
			missile[i].t=chrono;
  			missile[i].target=target;
  			missile[i].seeker=seeker;
		    break;
		}
	}
}

void proceed_missile()
{
    int i, u;
    VECTOR heading; float dist, speed;
    MATRIX_f m;
	for (i=0; i<AL.missile.anz; i++)
	{
	   if(missile[AL.missile.i[i]].seeker)
	   {
	   get_heading(&missile[AL.missile.i[i]].m, &missile[AL.missile.i[i]].pos, &unit[missile[AL.missile.i[i]].target].pos, &heading);
	   if (heading.x>0.01)
	   { 
	      get_y_rotate_matrix_f(&m,-MISSILETURN*fmult);
          matrix_mul_f(&missile[AL.missile.i[i]].m, &m, &missile[AL.missile.i[i]].m);
       }
       if (heading.x<-0.01)
       {
          get_y_rotate_matrix_f(&m, MISSILETURN*fmult);
       ...
       if (heading.y>0.01)
       {
          get_x_rotate_matrix_f(&m, MISSILETURN*fmult);
          matrix_mul_f(&missile[AL.missile.i[i]].m, &m, &missile[AL.missile.i[i]].m);
       }
       if (heading.y<-0.01)
       {
       ...
       if (missile[AL.missile.i[i]].speed != MISSILEMAXSPEED) accelerate(MISSILEMAXSPEED, &missile[AL.missile.i[i]].speed, MISSILEACC);
       speed=missile[AL.missile.i[i]].speed*fmult;
       missile[AL.missile.i[i]].pos.x-=missile[AL.missile.i[i]].m.v[2][0]*speed;
       missile[AL.missile.i[i]].pos.y-=missile[AL.missile.i[i]].m.v[2][1]*speed;
       missile[AL.missile.i[i]].pos.z-=missile[AL.missile.i[i]].m.v[2][2]*speed;
       if (chrono-missile[AL.missile.i[i]].t>5500) missile[AL.missile.i[i]].active=0;
	}
}
...
void set_myship_r()
{
	...
	glViewport(0,  113, SCREEN_W+init.aap, SCREEN_H-113);
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluPerspective(init.zoom, init.aspect, init.near_clip, init.far_clip);
	glMatrixMode(GL_MODELVIEW);
...
void draw_integer(int number)
{
   ...
   b[0]=str[0];
    ...
    {
       a=atoi(b);
 	   for (p = 0; p < model[9].o3d[a].poly_anz; p++)
	   { 
		  glVertexPointer(3, GL_FLOAT, 0, model[9].o3d[a].vert);
		  glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, model[9].o3d[a].poly[p].i);
	   }
	   glTranslatef(8,0,0);
	   i++;
       b[0]=str[i];
...
//	   glColor3ubv(col[c][15]);
	...
	{
	   if(!missile[AL.missile.i[i]].property)
	   {
	   glPushMatrix();
...
void proceed_myship()
{
	...
	crosspos.x+=unit[0].m.v[2][0]*speed;
	crosspos.y+=unit[0].m.v[2][1]*speed;
	crosspos.z+=unit[0].m.v[2][2]*speed;
	if (crosspos.x > CROSSGRID) crosspos.x-=CROSSGRID;	if (crosspos.x < -CROSSGRID) crosspos.x+=CROSSGRID;
	if (crosspos.y > CROSSGRID) crosspos.y-=CROSSGRID;	if (crosspos.y < -CROSSGRID) crosspos.y+=CROSSGRID;
	if (crosspos.z > CROSSGRID) crosspos.z-=CROSSGRID;	if (crosspos.z < -CROSSGRID) crosspos.z+=CROSSGRID;
...
void init_collision_model(short mod, short mod2)
{
    ...
    colmodel[mod2]->addTriangle(&model[mod].o3d[0].vert[model[mod].o3d[0].poly[p].i[0]].x,
                                &model[mod].o3d[0].vert[model[mod].o3d[0].poly[p].i[1]].x,
                                &model[mod].o3d[0].vert[model[mod].o3d[0].poly[p].i[2]].x);
