...
{
    short u;
    for(u=1; u<MAXUNITS; u++) if(!unit[u].active) break;
    unit[u].active=TRUE;
    return(u);
}
    ...
    for(i=0; i<AL.shot.anz; i++)
    {
       for(u=0;u<AL.u.anz;u++)
       {
          if (shot[AL.shot.i[i]].property!=AL.u.i[u])
  		  if (check_col(&shot[AL.shot.i[i]].pos, &unit[AL.u.i[u]].pos, model[unit[AL.u.i[u]].model].radius + (shot[AL.shot.i[i]].speed + unit[AL.u.i[u]].speed_ist.z)*fmult));
  		  {
             get_heading(&shot[AL.shot.i[i]].m, &shot[AL.shot.i[i]].pos, &unit[AL.u.i[u]].pos, &heading[0]);
             get_heading(&unit[AL.u.i[u]].m, &unit[AL.u.i[u]].pos, &shot[AL.shot.i[i]].pos, &heading[1]);
  		     speed=heading[0].z*shot[AL.shot.i[i]].speed+heading[1].z*unit[AL.u.i[u]].speed_ist.z;
  		     checksperframe=speed*fmult/CHECKDISTANCE;
  		     if(checksperframe<1)checksperframe=1;   
             vo.x=shot[AL.shot.i[i]].m.v[2][0];
             vo.y=shot[AL.shot.i[i]].m.v[2][1];
             vo.z=shot[AL.shot.i[i]].m.v[2][2];
             pos1=unit[AL.u.i[u]].pos;
             pos2=shot[AL.shot.i[i]].pos;
             speed1=unit[AL.u.i[u]].speed_ist.z/checksperframe;
             speed2=shot[AL.shot.i[i]].speed/checksperframe;
             
  		     for(j=0;j<checksperframe;j++)
             {
                get_translation_matrix_f(&tm0, pos1.x, pos1.y, pos1.z);
                matrix_mul_f(&unit[AL.u.i[u]].m, &tm0, &tm0);
                allegro_gl_MATRIX_f_to_GLfloat(&tm0, glm0);
                colmodel[unit[AL.u.i[u]].model]->setTransform (glm0);
                hit=colmodel[unit[u].model]->rayCollision (&pos2.x, &vo.x, 0, 0, SHOTLENGTH+CHECKDISTANCE);
                if(hit)break;
                pos1.x-=unit[AL.u.i[u]].m.v[2][0]*speed1;
                pos1.y-=unit[AL.u.i[u]].m.v[2][1]*speed1;
                pos1.z-=unit[AL.u.i[u]].m.v[2][2]*speed1;
                pos2.x-=unit[AL.shot.i[i]].m.v[2][0]*speed2;
                pos2.y-=unit[AL.shot.i[i]].m.v[2][1]*speed2;
                pos2.z-=unit[AL.shot.i[i]].m.v[2][2]*speed2;
             }
...
// 		        start_explode(&shot[AL.shot.i[i]].pos, &unit[AL.u.i[u]].m, &unit[AL.u.i[u]].speed_ist.z, shot[AL.shot.i[i]].property);
                ...
                {
                   damage_unit(AL.u.i[u], SHOTDAMAGE);
...
{
   short i=create_unit();
   unit[i].type=UNIT_MISSILE;
   unit[i].status=status;
   unit[i].pos=unit[u].pos;
   unit[i].m=unit[u].m;
   unit[i].target=unit[u].target;
   unit[i].speed_ist.z=unit[u].speed_ist.z+MISSILESTARTSPEED;
   unit[i].speed_soll.z=unit[u].speed_ist.z+MISSILEMAXSPEED;
   unit[i].t=chrono;
}
...
//	init_collision_model(3, 4);
	...
	u=2;
	unit[u].status=AI_SELFROTATE;
	unit[u].type=UNIT_OBJECT;
	unit[u].active = 1;
	unit[u].model = 5;
	unit[u].m =identity_matrix_f;
	unit[u].pos.x = 300;
