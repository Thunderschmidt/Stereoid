...
void get_unit_pointers()
{
   short u;
   for(u=0; u<MAXUNITS; u++)un[u]=&unit[u];
//   *un[0].type=UNIT_FIGHTER;
}
...
void get_headings()
{
    ...
    for (i=1; i<AL.u.anz; i++)
    {
       get_heading(&unit[0].m, &camera.pos, &unit[AL.u.i[i]].pos, &unit[AL.u.i[i]].heading);
    }
    for (i=0; i<shot_anz; i++)
    {
...
void collision_check()
{
    ...
    for(i=0; i<shot_anz; i++)
    {
       ...
       {
          if (shot[i].property!=AL.u.i[u])
  		  if (check_col(&shot[i].pos, &unit[AL.u.i[u]].pos, model[unit[AL.u.i[u]].model].radius + (shot[i].speed + unit[AL.u.i[u]].speed_ist.z)*fmult));
  		  {  
  		     speed=get_approaching_speed(&shot[i].m, &shot[i].pos, &shot[i].speed, &unit[AL.u.i[u]].m, &unit[AL.u.i[u]].pos, &unit[AL.u.i[u]].speed_ist.z);
  		     ...
  		     for(j=0;j<checksperframe;j++)
             {
  		     ...
  		     if(hit)
  		     {  
// 		        start_explode(&shot[i].pos, &unit[AL.u.i[u]].m, &unit[AL.u.i[u]].speed_ist.z, shot[i].property);
  		        shot[i].active=0;
  		        if (AL.u.i[u] != 0)play_3D_sound(&shot[i].pos, sound[2]);
